...
- **Transações multi-tabela** via `prisma.$transaction(async (tx) => { ... })` (veja `src/app/(auth)/actions.ts` no registro de usuário, e `src/app/(app)/lancamentos/import-actions.ts` na importação em lote). Quando uma função helper precisa rodar dentro de uma transação, ela recebe o client (`PrismaClient | Prisma.TransactionClient`) como parâmetro em vez de importar `prisma` direto — siga esse padrão em novos helpers.
...
1. Leia o schema e as actions relacionadas antes de propor mudança — confirme campos/relations reais, não assuma. Leia só o que a tarefa toca; não explore o projeto inteiro "pra se ambientar".
2. Ao receber um plano do `techlead` para validação, ele já deve ter trazido os trechos relevantes que leu — reaproveite isso em vez de reler os mesmos arquivos do zero. Aponte de forma objetiva: queries sem escopo de `userId`, validação Zod faltante, falta de transação onde há múltiplas escritas relacionadas, e migrations que faltam (`@@unique`/`@@index` necessários).
...
4. Rode `npm run build` (ou ao menos `tsc --noEmit` + `npm run lint`) **uma vez ao final** da mudança, não repetidamente depois de cada edição pequena — verificação intermediária só quando precisar decidir o próximo passo.
